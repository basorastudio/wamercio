...
checks = {
    ...
    'dashboard only counts buyers': "WHERE s.user_id=$1 AND EXISTS (SELECT 1 FROM orders o WHERE o.customer_id=cu.id AND o.status<>'canceled')" in server,
    'customer stats ignore canceled orders': "count(*) FILTER (WHERE status<>'canceled')::int cnt" in server,
    ...
    'conversation details uses purchase existence': "SELECT EXISTS(SELECT 1 FROM orders WHERE customer_id=$1 AND status<>'canceled')" in server[server.find('func (s *Server) conversationDetails'):server.find('func (s *Server) saveConversationCustomer')],
    'whatsapp event only links purchased customer': "AND EXISTS (SELECT 1 FROM orders o WHERE o.customer_id=customers.id AND o.status<>'canceled')" in server[server.find('func (s *Server) whatsappEvent'):server.find('func (s *Server) whatsappProfile')],
