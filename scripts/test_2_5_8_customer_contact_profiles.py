...
checks = {
    ...
    'conversation details recognizes linked registered customer without purchase': 'isCustomer := customerID != ""' in server[server.find('func (s *Server) conversationDetails'):server.find('func (s *Server) saveConversationCustomer')],
    'whatsapp event resolves registered platform customer': 'registeredCustomerForWhatsApp' in server[server.find('func (s *Server) whatsappEvent'):server.find('func (s *Server) whatsappProfile')],
