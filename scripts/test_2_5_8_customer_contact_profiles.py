...
checks = {
    ...
    'conversation details uses purchase existence': "SELECT EXISTS(SELECT 1 FROM orders WHERE customer_id=$1 AND status<>'canceled' AND flow_type<>'quote')" in server[server.find('func (s *Server) conversationDetails'):server.find('func (s *Server) saveConversationCustomer')],
    'whatsapp event only links purchased customer': "AND EXISTS (SELECT 1 FROM orders o WHERE o.customer_id=customers.id AND o.status<>'canceled' AND o.flow_type<>'quote')" in server[server.find('func (s *Server) whatsappEvent'):server.find('func (s *Server) whatsappProfile')],
