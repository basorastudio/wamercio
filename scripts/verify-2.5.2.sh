...
[ "$(cat VERSION)" = "2.5.2" ] || fail 'VERSION'
grep -q '"version": "2.5.2"' apps/web/package.json || fail 'web version'
grep -q 'wamercio-store-v2.5.2' apps/web/public/sw.js || fail 'PWA cache'

python3 scripts/test_2_5_2_multitenant_hosts.py
...
echo 'PASS: WAMERCIO 2.5.2 static verification'
