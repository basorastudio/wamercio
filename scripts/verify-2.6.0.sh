...
[ "$(cat VERSION)" = "2.6.0" ] || fail 'VERSION'
grep -q '"version": "2.6.0"' apps/web/package.json || fail 'web version'
grep -q 'wamercio-store-v2.6.0' apps/web/public/sw.js || fail 'PWA cache'
...
python3 scripts/test_2_5_8_storefront_purchase_order_details.py
...
sh scripts/verify-owner-tabs.sh
sh scripts/verify-convergence.sh
sh scripts/verify-api-compile-guards.sh
sh scripts/verify-web-settings-types.sh
...
[ -f services/api/migrations/000031_promotions_phase1.up.sql ] || fail 'migration 000031 up'
[ -f services/api/migrations/000031_promotions_phase1.down.sql ] || fail 'migration 000031 down'
...
echo 'PASS: WAMERCIO 2.6.0 static verification'
