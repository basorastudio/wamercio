...
[ "$(cat VERSION)" = "2.5.4" ] || fail 'VERSION'
grep -q '"version": "2.5.4"' apps/web/package.json || fail 'web version'
grep -q 'wamercio-store-v2.5.4' apps/web/public/sw.js || fail 'PWA cache'

python3 scripts/test_2_5_4_multitenant_hosts.py
python3 scripts/test_2_5_4_exact_tenant_routers.py
...
grep -q 'TENANT_ROOT_DOMAIN: ${TENANT_ROOT_DOMAIN:-ltd.do}' docker-compose.yml || fail 'domain router tenant root env'
...
echo 'PASS: WAMERCIO 2.5.4 static verification'
