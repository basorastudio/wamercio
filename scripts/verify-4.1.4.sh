...
[ "$(cat VERSION)" = "4.1.4" ] || fail VERSION
grep -q '"version": "4.1.4"' apps/web/package.json || fail 'web version'
grep -q 'wamercio-store-v4.1.4' apps/web/public/sw.js || fail 'PWA cache'
python3 scripts/test_4_1_4_calls_runtime.py
python3 scripts/test_4_1_3_build_scope.py
python3 scripts/test_3_0_0_conversation_center_pro.py
go run scripts/check_go_scope.go services/api/internal/httpapi services/whatsapp-bridge/internal/bridge services/whatsapp-bridge/internal/voip/call services/whatsapp-bridge/internal/wacall
python3 - <<'PY2'
...
PY3
echo 'gofmt: OK'; echo 'Shell syntax: OK'; echo 'PASS: WAMERCIO 4.1.4 release verification'
