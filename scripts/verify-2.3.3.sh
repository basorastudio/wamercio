...
[ "$(cat VERSION)" = "2.3.3" ] || fail 'VERSION'
grep -q '"version": "2.3.3"' apps/web/package.json || fail 'web version'
grep -q 'wamercio-store-v2.3.3' apps/web/public/sw.js || fail 'PWA cache'
...
python scripts/test_2_3_2_owner_business_flows.py
python scripts/test_2_3_3_public_registration_profile.py
...
[ -f services/api/migrations/000017_owner_whatsapp_business_identity.up.sql ] || fail 'migration 000017 up'
[ -f services/api/migrations/000017_owner_whatsapp_business_identity.down.sql ] || fail 'migration 000017 down'
...
echo 'PASS: WAMERCIO 2.3.3 static verification'
