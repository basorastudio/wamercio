...
type CallManager struct {
	...
	videoOutboundFrames    uint64
	videoInboundFrames     uint64
	lastInboundVideoAt     time.Time
...
func (m *CallManager) StartCall(ctx context.Context, callID string, peerJid types.JID, isVideo bool) error {
	...
	// Visible audit trail in hierrodelnorte slog (journalctl) — separate from whatsmeow's logger.
