...
func (m *CallManager) HandleCallAck(ctx context.Context, node *waBinary.Node) {
	...
	// early termination before the call starts. The outgoing side should only wait for the peer's
...
func incomingCallPeerName(outer, inner *waBinary.Node) string {
	if outer == nil && inner == nil {
		return ""
	}
	keys := []string{
		"notify", "pushname", "pushName", "display-name", "display_name",
		"verified-name", "verified_name", "business-name", "business_name", "name",
	}
	for _, n := range []*waBinary.Node{outer, inner} {
		if n == nil {
			continue
		}
		for _, key := range keys {
			if value := strings.TrimSpace(wanode.AttrString(n.Attrs, key)); isUsefulPeerName(value) {
				return value
			}
		}
	}
...
func isUsefulPeerName(value string) bool {
