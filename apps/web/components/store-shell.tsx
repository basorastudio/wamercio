...
function NavLink({n,onClick,collapsed}:{n:any;onClick?:()=>void;collapsed?:boolean}){
  ...
  return <Link href={n.href} data-active={active?'true':'false'} title={n.label} onClick={onClick} className={`merchant-nav-link group relative mx-2 flex items-center text-sm transition ${collapsed?'justify-center rounded-2xl px-0 py-3':'gap-3 rounded-r-2xl px-4 py-2.5'} ${active?'bg-brand-50 font-medium text-brand-700':'text-[#777c96] hover:bg-[#fafbfe] hover:text-ink-900'}`}>
...
export default function StoreShell({children,title,subtitle,actions,context,fullHeight=false,hideHeader=false,structuralRightRail=false}:{children:React.ReactNode;title:string;subtitle?:string;actions?:React.ReactNode;context?:React.ReactNode;fullHeight?:boolean;hideHeader?:boolean;structuralRightRail?:boolean}){
 ...
 return <div data-sidebar-collapsed={collapsed?'true':'false'} className="merchant-ui group/shell min-h-dvh bg-[#f7f9fc] pb-[calc(72px+env(safe-area-inset-bottom))] text-ink-900 lg:pb-0">
  ...
  <aside className={`merchant-sidebar fixed inset-y-0 left-0 z-50 flex flex-col border-r border-[#eceef4] bg-white transition-[width,transform] duration-200 lg:translate-x-0 ${collapsed?'w-[88px]':'w-[258px]'} ${drawer?'translate-x-0':'-translate-x-full'}`}>
   ...
   <div className="merchant-sidebar-footer border-t border-[#f0f1f5] p-3">
  ...
  <div className={`transition-[padding] duration-200 ${collapsed?'lg:pl-[88px]':'lg:pl-[258px]'}`}>
   {!hideHeader&&<header className="merchant-topbar sticky top-0 z-30 border-b border-[#eceef4] bg-white/95 backdrop-blur">
