...
export default function StoreShell({children,title,subtitle,actions,context,fullHeight=false,hideHeader=false,structuralRightRail=false}:{children:React.ReactNode;title:string;subtitle?:string;actions?:React.ReactNode;context?:React.ReactNode;fullHeight?:boolean;hideHeader?:boolean;structuralRightRail?:boolean}){
 ...
 const[incomingCall,setIncomingCall]=useState<{id:string;store_id:string;phone:string;display_name:string}|null>(null)
 ...
 useEffect(()=>{if(typeof window==='undefined')return;const onIncoming=(event:Event)=>{const d=(event as CustomEvent).detail||{};setIncomingCall({id:String(d.id||''),store_id:String(d.store_id||''),phone:String(d.phone||''),display_name:String(d.display_name||'')})};const onClear=()=>setIncomingCall(null);window.addEventListener('wamercio:incoming-call-attention',onIncoming as EventListener);window.addEventListener('wamercio:call-attention-clear',onClear);return()=>{window.removeEventListener('wamercio:incoming-call-attention',onIncoming as EventListener);window.removeEventListener('wamercio:call-attention-clear',onClear)}},[])
 ...
 const launchSoftphone=()=>{const id=incomingCall?.store_id||activeStoreId||((typeof window!=='undefined'&&localStorage.getItem('wamercio_store_id'))||'')||primaryStore?.id||'';if(id)setActiveStoreId(id);if(typeof window!=='undefined'){window.dispatchEvent(new CustomEvent('wamercio:call-attention-clear'));window.dispatchEvent(new CustomEvent('wamercio:open-softphone',{detail:{store_id:id,phone:incomingCall?.phone||'',display_name:incomingCall?.display_name||'',picture_in_picture:true}}))}}
 ...
 return <div data-sidebar-collapsed={collapsed?'true':'false'} className="group/shell min-h-dvh bg-[#f7f9fc] pb-[calc(72px+env(safe-area-inset-bottom))] text-ink-900 lg:pb-0">
  ...
  <aside className={`fixed inset-y-0 left-0 z-50 flex flex-col border-r border-[#eceef4] bg-white transition-[width,transform] duration-200 lg:translate-x-0 ${collapsed?'w-[88px]':'w-[258px]'} ${drawer?'translate-x-0':'-translate-x-full'}`}>
   ...
   <div className="border-t border-[#f0f1f5] p-3">
    <button type="button" onClick={launchSoftphone} title={incomingCall?'Abrir llamada entrante':'Abrir softphone'} aria-label={incomingCall?'Abrir llamada entrante':'Abrir softphone'} className={`relative mb-2 flex w-full items-center rounded-2xl border transition ${incomingCall?'border-emerald-300 bg-emerald-50 text-emerald-800 shadow-[0_0_0_3px_rgba(16,185,129,.08)]':'border-brand-100 bg-brand-50/70 text-brand-700 hover:border-brand-200 hover:bg-brand-50'} ${collapsed?'justify-center p-2.5':'gap-3 px-3 py-2.5'}`}><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl text-white shadow-sm ${incomingCall?'animate-pulse bg-emerald-500':'bg-brand-500'}`}><PhoneCall className="h-4 w-4"/></span>{incomingCall&&<span className="absolute right-2 top-2 h-2.5 w-2.5 rounded-full bg-rose-500 ring-2 ring-white"/>}{!collapsed&&<span className="min-w-0 flex-1 text-left"><span className="block text-xs font-semibold">{incomingCall?'Llamada entrante':'Abrir softphone'}</span><span className={`mt-0.5 block truncate text-[10px] font-medium ${incomingCall?'text-emerald-700/75':'text-brand-600/70'}`}>{incomingCall?(incomingCall.display_name||incomingCall.phone||'Toca para contestar'):'Llamadas WhatsApp'}</span></span>}</button>
