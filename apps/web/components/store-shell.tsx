...
export default function StoreShell({children,title,subtitle,actions}:{children:React.ReactNode;title:string;subtitle?:string;actions?:React.ReactNode}){
 ...
 return <div className="min-h-dvh bg-[#f7f9fc] pb-[calc(72px+env(safe-area-inset-bottom))] text-ink-900 lg:pb-0">
  ...
  <aside className={`fixed inset-y-0 left-0 z-50 flex flex-col border-r border-[#eceef4] bg-white transition-[width,transform] duration-200 lg:translate-x-0 ${collapsed?'w-[88px]':'w-[258px]'} ${drawer?'translate-x-0':'-translate-x-full'}`}>
   ...
   <div className="border-b border-[#f0f1f5] px-3 py-3 hidden lg:block">
    <button type="button" onClick={toggleCollapsed} className={`flex w-full items-center rounded-2xl border border-[#e8ebf2] bg-[#fafbfe] px-3 py-2.5 text-sm font-medium text-[#6f758d] transition hover:border-brand-200 hover:bg-brand-50 ${collapsed?'justify-center':''}`}>
      {collapsed?<ChevronRight className="h-4 w-4 text-brand-600"/>:<><ChevronLeft className="h-4 w-4 text-brand-600"/><span className="ml-2">Contraer menú</span></>}
    </button>
   </div>
  ...
  <div className={`transition-[padding] duration-200 ${collapsed?'lg:pl-[88px]':'lg:pl-[258px]'}`}>
   <header className="sticky top-0 z-30 border-b border-[#eceef4] bg-white/95 backdrop-blur">
    <div className="flex min-h-[66px] items-center gap-3 px-3 sm:px-5 lg:px-6 xl:px-7">
      ...
      <div className="min-w-0 flex-1"><h1 className="truncate text-lg font-semibold tracking-tight text-ink-900 sm:text-xl">{title}</h1>{subtitle&&<p className="hidden truncate text-xs text-[#9a9fb5] sm:block">{subtitle}</p>}</div><div className="flex shrink-0 items-center gap-2">{actions}</div>
    </div>
   </header>
   <main className="mx-auto w-full max-w-[1700px] p-3 sm:p-5 lg:p-6 xl:p-7">{children}</main>
  </div>
