...
import { LayoutDashboard, Store, Boxes, Tags, ShoppingBag, TicketPercent, Truck, MessageCircleMore, MessagesSquare, Layers3, Settings, LogOut, Menu, X, ChevronDown, UserRound } from 'lucide-react'
...
const nav=[
  ...
  {href:'/plans',label:'Planes',icon:Layers3},
 ...
 return <div className="min-h-screen bg-slate-50">
  ...
  <aside className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-slate-200 bg-white transition-transform lg:translate-x-0 ${open?'translate-x-0':'-translate-x-full'}`}>
   ...
   <nav className="flex-1 space-y-1 overflow-y-auto p-4">
    <p className="px-3 pb-2 pt-2 text-[11px] font-bold uppercase tracking-[.14em] text-slate-400">Gestión</p>
    {nav.map(n=>{const Icon=n.icon;const active=path===n.href||path.startsWith(n.href+'/');return <Link key={n.href} href={n.href} onClick={()=>setOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active?'bg-brand-50 text-brand-700':'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}><Icon className="h-[18px] w-[18px]"/><span>{n.label}</span></Link>})}
   </nav>
   <div className="border-t border-slate-100 p-4">
    <div className="flex items-center gap-3 rounded-xl bg-slate-50 p-3"><div className="grid h-9 w-9 place-items-center rounded-full bg-white text-slate-500"><UserRound className="h-4 w-4"/></div><div className="min-w-0 flex-1"><div className="truncate text-sm font-semibold">{me?.name||'Cargando...'}</div><div className="truncate text-xs text-slate-500">{me?.email||''}</div></div><button title="Cerrar sesión" onClick={logout} className="text-slate-400 hover:text-rose-600"><LogOut className="h-4 w-4"/></button></div>
   </div>
  </aside>
