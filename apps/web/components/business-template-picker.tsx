...
export default function BusinessTemplatePicker({value,onChange,compact=false}:{value:string;onChange:(slug:string,template?:BusinessTemplate)=>void;compact?:boolean}){
  ...
  return <div>
    ...
    <div className={`grid gap-3 ${compact?'sm:grid-cols-2':'sm:grid-cols-2 lg:grid-cols-3'}`}>{filtered.map(t=>{
      ...
      const theme=getStoreTheme(String(t.settings?.visual_theme||'minimal-shop'))
      return <button type="button" key={t.id} onClick={()=>onChange(t.slug,t)} className={`group relative overflow-hidden rounded-[22px] border text-left transition ${active?'border-brand-400 bg-brand-50/65 shadow-[0_8px_24px_rgba(54,179,133,.12)]':'border-[#e9ebf1] bg-white hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-md'}`}>
        <div className="h-16 p-2.5" style={{background:`linear-gradient(135deg,${theme.config.colors.background},${theme.config.colors.secondary})`}}><div className="flex h-full gap-2"><div className="w-1/4 rounded-xl" style={{background:theme.config.colors.primary}}/><div className="grid flex-1 grid-cols-2 gap-2"><div className="rounded-lg" style={{background:theme.config.colors.surface}}/><div className="rounded-lg" style={{background:theme.config.colors.surface}}/></div></div></div>
        <div className="p-4"><div className="flex items-start gap-3"><span className={`grid h-12 w-12 shrink-0 place-items-center rounded-2xl text-2xl ${active?'bg-white':'bg-[#f7f8fb]'}`}>{t.icon||'✨'}</span><div className="min-w-0 flex-1"><div className="flex items-start gap-2"><h3 className="font-semibold text-ink-900">{t.name}</h3>{t.is_featured&&<span className="rounded-full bg-amber-50 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-amber-700">Popular</span>}</div><p className="mt-1 line-clamp-2 text-xs leading-5 text-[#8d92aa]">{t.description}</p></div>{active?<span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-brand-500 text-white"><Check className="h-4 w-4"/></span>:<ChevronRight className="h-4 w-4 shrink-0 text-[#c0c4d0] transition group-hover:translate-x-0.5 group-hover:text-brand-600"/>}</div>
      ...
      <div className="mt-3 flex items-center justify-between border-t border-[#eef0f4] pt-3 text-[10px] font-semibold"><span style={{color:theme.config.colors.primary}}>{theme.name}</span><span className="text-[#a0a5b7]">{t.engine}</span></div></div>
      </button>
    })}</div>
