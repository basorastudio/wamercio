...
type LocalTab='general'|'design'|'sales'|'hours'
export default function SettingsNav({active,onSelect}:{active:SettingsTab;onSelect?:(tab:LocalTab)=>void}){
  return <aside className="card h-fit p-2">{items.map(item=>{
    ...
    if((item.id==='general'||item.id==='design'||item.id==='sales'||item.id==='hours')&&onSelect)return <button type="button" key={item.id} onClick={()=>onSelect(item.id)} className={cls}><I className="h-4 w-4"/>{item.label}</button>
