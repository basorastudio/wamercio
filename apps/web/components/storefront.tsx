...
export default function Storefront(){
  ...
  const paymentOptions=['cash','cash_on_delivery','bank_transfer'].filter(k=>methods[k])
  ...
  const cartNotes=<section data-testid="storefront-cart-notes" className="p-3.5 sm:p-4" style={{...surfaceStyle(t),boxShadow:'none'}}><div className="text-[10px] font-bold uppercase tracking-[.14em]" style={{color:t.colors.primary}}>3 · Notas</div><label className="label mt-2">Indicaciones para el pedido <span className="font-normal" style={{color:t.colors.muted}}>(opcional)</span></label><textarea className="field min-h-20" placeholder="Ej.: llamar al llegar, sin servilletas..." value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/></section>
