...
export type Order={id:string;number:number;customer_name:string;customer_phone:string;total:number;payment_method:string;payment_status:string;cash_change_requested?:boolean;cash_tendered?:number;status:string;source:string;delivery_type?:'pickup'|'delivery';flow_type?:'order'|'reservation'|'quote';custom_fields?:Record<string,any>;created_at:string}
