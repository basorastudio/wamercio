...
export type Order={id:string;number:number;customer_name:string;customer_phone:string;total:number;payment_method:string;payment_status:string;status:string;source:string;delivery_type?:'pickup'|'delivery';flow_type?:'order'|'reservation'|'quote';custom_fields?:Record<string,any>;created_at:string}
export type Customer={id:string;store_id:string;name:string;phone:string;address:string;notes:string;status:'active'|'blocked';order_count:number;total_spent:number;last_order_at?:string|null;created_at:string;contact_type?:'customer';whatsapp_name?:string;profile_picture_url?:string}
