...
const h264CodecFromAnnexB = (data: Uint8Array): string => {
  for (let i = 0; i + 7 < data.length; i += 1) {
    let p = -1
    if (data[i] === 0 && data[i + 1] === 0 && data[i + 2] === 1) p = i + 3
    else if (data[i] === 0 && data[i + 1] === 0 && data[i + 2] === 0 && data[i + 3] === 1) p = i + 4
    if (p < 0 || p + 3 >= data.length || (data[p] & 31) !== 7) continue
    const profile = data[p + 1].toString(16).padStart(2, '0')
    const compatibility = data[p + 2].toString(16).padStart(2, '0')
    const level = data[p + 3].toString(16).padStart(2, '0')
    return `avc1.${profile}${compatibility}${level}`.toUpperCase().replace('AVC1.', 'avc1.')
  }
  return 'avc1.42E01F'
}

export async function openWamercioCallAudio(callId: string, onUnexpectedClose?: () => void, onRemoteAudio?: () => void, onRemoteVideo?: () => void): Promise<WamercioBrowserCall> {
  ...
  const videoDc = pc.createDataChannel(H264_CHANNEL_LABEL, { ordered: true })
  ...
  let decoderCodec = ''
  let remoteVideoConfirmed = false
  ...
  const getDecoder = (codec: string) => {
    ...
    decoder = new VideoDecoderCtor({
      output: (frame: any) => {
        ...
        if (!remoteVideoConfirmed) {
          remoteVideoConfirmed = true
          onRemoteVideo?.()
        }
  ...
  videoDc.onmessage = async (event: MessageEvent) => {
    ...
    const codec = key ? h264CodecFromAnnexB(au) : (decoderCodec || 'avc1.42E01F')
    decodeStarted = true
    try {
      decodeTimestamp += 66666
      getDecoder(codec)?.decode(new EncodedVideoChunkCtor({ type: key ? 'key' : 'delta', timestamp: decodeTimestamp, data: au }))
    } catch {
      decodeStarted = false
    }
  }
