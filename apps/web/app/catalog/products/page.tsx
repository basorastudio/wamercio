...
import type {Category,PriceOption,Product,Store} from '@/lib/types'
import {Plus,Boxes,Pencil,Trash2,ChevronDown,Sparkles,Package2,Image as ImageIcon,ListFilter} from 'lucide-react'
...
const blank={name:'',slug:'',sku:'',tag:'',description:'',image_url:'',category_id:'',price:0,compare_price:null as number|null,stock:null as number|null,track_stock:false,variants:[] as PriceOption[],extras:[] as PriceOption[],attributes:{} as Record<string,any>,is_featured:false,sort_order:0,is_active:true}

export default function Products(){
 const[store,setStore]=useState(''),[currentStore,setCurrentStore]=useState<Store|null>(null),[rows,setRows]=useState<Product[]>([]),[cats,setCats]=useState<Category[]>([]),[attrs,setAttrs]=useState<AttributeDef[]>([]),[loading,setLoading]=useState(false),[open,setOpen]=useState(false),[edit,setEdit]=useState<Product|null>(null),[form,setForm]=useState<any>(blank),[err,setErr]=useState(''),[search,setSearch]=useState(''),[up,setUp]=useState(false),[advanced,setAdvanced]=useState(false)
 const load=()=>{if(!store){setRows([]);setCats([]);setAttrs([]);return};setLoading(true);Promise.all([api<Product[]>(`/products?store_id=${store}`),api<Category[]>(`/categories?store_id=${store}`),api<AttributeDef[]>(`/product-attributes?store_id=${store}`)]).then(([p,c,a])=>{setRows(p);setCats(c);setAttrs(a)}).finally(()=>setLoading(false))}
 useEffect(load,[store])
 const filtered=useMemo(()=>rows.filter(x=>(x.name+' '+(x.description||'')).toLowerCase().includes(search.toLowerCase())),[rows,search])
 const start=(x?:Product)=>{setEdit(x||null);setForm(x?{...x,variants:x.variants||[],extras:x.extras||[],attributes:x.attributes||{}}:{...blank,attributes:{},sort_order:(rows.length+1)*10});setAdvanced(!!(x&&(x.track_stock||x.variants?.length||x.extras?.length||!x.is_active)));setErr('');setOpen(true)}
