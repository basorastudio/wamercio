...
import {Eye,MapPin,ShoppingBag,WalletCards,MessageCircleMore,ArrowLeft,UsersRound,Gift,Ban,LockOpen,ShieldAlert,FileCheck2} from 'lucide-react'

type ModalView='detail'|'profile'|'block'|'unblock'|null
...
export default function Customers(){
 ...
 const startBlock=(c:Customer)=>{setActionCustomer(c);setBlockReason('');setErr('');setView(c.status==='blocked'?'unblock':'block')}
 const closeModal=()=>{setView(null);setDetail(null);setActionCustomer(null);setBlockReason('');setErr('')}
 ...
 const modalTitle=view==='block'?'Bloquear cliente':view==='unblock'?'Desbloquear cliente':view==='profile'?'Detalles del cliente':detail?.name||'Cliente'
