...
import {BarChart3,CalendarDays,ReceiptText,ShoppingBag,UsersRound,WalletCards,Trophy,Tag} from 'lucide-react'

type AnalyticsData={
 range:string;from:string;metrics:{orders:number;revenue:number;average_ticket:number;customers:number};
 ...
 return <StoreShell title="Analítica" subtitle="Ventas, clientes y rendimiento comercial en un solo lugar" context={<StoreSelector value={store} onChange={setStore}/>} actions={<div className="inline-flex rounded-2xl border border-[#e8ebf1] bg-white p-1">{[['7d','7 días'],['30d','30 días'],['90d','90 días']].map(([value,label])=><button key={value} disabled={!store} onClick={()=>setRange(value)} className={`rounded-xl px-3 py-2 text-xs font-semibold transition ${range===value?'bg-brand-50 text-brand-700':'text-[#7f859b] hover:text-ink-900'}`}>{label}</button>)}</div>}>
  {!store?<PageEmpty title="Selecciona una tienda" detail="Elige una tienda para consultar su rendimiento."/>:loading?<Loading/>:<div className="space-y-5">{err&&<Alert text={err}/>} {data&&<>
   <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
    {label:'Ventas',value:money(data.metrics.revenue),detail:`${data.metrics.orders} pedidos`,icon:WalletCards},
    {label:'Pedidos',value:String(data.metrics.orders),detail:'Pedidos no cancelados',icon:ShoppingBag},
    {label:'Ticket promedio',value:money(data.metrics.average_ticket),detail:'Promedio por pedido',icon:ReceiptText},
    {label:'Clientes',value:String(data.metrics.customers),detail:'Clientes únicos',icon:UsersRound},
   ].map(card=>{const I=card.icon;return <article className="card p-5" key={card.label}><div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.1em] text-[#9aa0b4]">{card.label}</p><p className="mt-2 text-2xl font-semibold tracking-tight text-ink-900">{card.value}</p><p className="mt-1 text-xs text-[#8f95a9]">{card.detail}</p></div><span className="grid h-11 w-11 place-items-center rounded-2xl bg-brand-50 text-brand-600"><I className="h-5 w-5"/></span></div></article>})}</section>
