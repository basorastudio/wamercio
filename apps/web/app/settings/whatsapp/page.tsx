  ...
  return <StoreShell title="Ajustes" subtitle="Configura tu comercio sin opciones innecesarias" context={<StoreSelector value={store} onChange={setStore}/>}> 
    {!store?<PageEmpty title="Selecciona una tienda" detail="Cada tienda mantiene su propia conexión de mensajería."/>:<div className="grid gap-5 xl:grid-cols-[190px_minmax(0,1fr)_330px]">
      ...
      <section className="card p-6 sm:p-8">{error&&<Alert text={error}/>}<div className="flex min-h-[440px] flex-col items-center justify-center text-center">
        <div className={`grid h-16 w-16 place-items-center rounded-3xl ${connected?'bg-emerald-50 text-emerald-600':'bg-[#f1f2f6] text-[#8d92aa]'}`}><Link2 className="h-8 w-8"/></div>
        <p className="section-kicker mt-5">Conexión</p>
        <h2 className="mt-1 text-2xl font-semibold text-ink-900">{connected?'Conexión activa':'Conecta tu cuenta'}</h2>
        <p className="mt-2 max-w-lg text-sm leading-6 text-[#8d92aa]">{connected?`Dispositivo WAMERCIO activo${state.phone?` con +${state.phone}`:''}. Ya puedes atender mensajes desde la sección WhatsApp.`:'Abre WhatsApp → Dispositivos vinculados → Vincular un dispositivo y escanea el código QR.'}</p>
        {state.qr&&!connected?<div className="mt-6 rounded-3xl border border-[#e9ebf2] bg-white p-5 shadow-soft"><QRCodeSVG value={state.qr} size={250} level="M" includeMargin/><p className="mt-3 text-xs text-[#a2a6b8]">El código se renueva automáticamente mientras esperas la vinculación.</p></div>:null}
      ...
      <aside className="space-y-4 xl:sticky xl:top-[82px] xl:self-start"><div className="card p-5"><ShieldCheck className="h-6 w-6 text-brand-600"/><h3 className="mt-3 font-semibold">Dispositivo WAMERCIO</h3><p className="mt-2 text-sm leading-6 text-[#8d92aa]">Las nuevas vinculaciones se registran con el nombre <strong className="text-[#3b4a54]">WAMERCIO</strong> y se mantienen supervisadas mientras el servicio esté en línea.</p></div><div className="card p-5"><Clock3 className="h-6 w-6 text-brand-600"/><h3 className="mt-3 font-semibold">Estado de la conexión</h3><div className="mt-3 space-y-2 text-sm"><div className="flex justify-between"><span className="text-[#8d92aa]">Dispositivo</span><strong>WAMERCIO</strong></div><div className="flex justify-between"><span className="text-[#8d92aa]">Conexión</span><strong className="capitalize">{connected?'Conectado':state.status||'desconectado'}</strong></div><div className="flex justify-between"><span className="text-[#8d92aa]">Actividad</span><strong className={connected?'text-emerald-600':'text-[#8d92aa]'}>{connected?'Supervisada':'Sin sesión'}</strong></div></div></div></aside>
    </div>}
  </StoreShell>
}
