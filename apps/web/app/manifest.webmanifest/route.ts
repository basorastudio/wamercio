...
export async function GET(req:NextRequest){
  ...
  if(host&&!platformHost(host)){
    manifest={...manifest,name:'Mi tienda',short_name:'Mi tienda',description:'Catálogo y pedidos en línea.',icons:[{src:`/tenant-icon.svg?tenant=${encodeURIComponent(host)}&v=2.8.3`,sizes:'any',type:'image/svg+xml',purpose:'any maskable'}]}
    try{
      ...
      if(res.ok){
        ...
        const fallbackIcon=`/tenant-icon.svg?tenant=${encodeURIComponent(host)}&v=2.8.3`
