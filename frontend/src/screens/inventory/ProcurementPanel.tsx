...
const ProcurementPanel = ({ storeId, products = [] }: { storeId: string; products: any[] }) => {
  ...
  return (
    <div className="space-y-5 p-4 md:p-6">
      ...
      {receiveOrder && <ReceiveModal order={receiveOrder} onClose={() => setReceiveOrder(null)} onSaved={async () => { setReceiveOrder(null); await load(); window.dispatchEvent(new CustomEvent('wamercio:data-changed')); }} />}
...
const PolicyModal = ({ product, suppliers, onClose, onSaved }: any) => {
  ...
  const [error, setError] = useState('');
  const save = async () => { try { await api.patch(`/products/${product.id}/reorder-policy`, form); window.dispatchEvent(new CustomEvent('wamercio:data-changed')); await onSaved(); } catch (err: any) { setError(err?.message || 'No se pudo guardar la política'); } };
